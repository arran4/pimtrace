...
func TestWriteMBoxStream_WithData(t *testing.T) {
	...
	if err := data.WriteMBoxStream(tmpFile, "file.mbox"); err != nil { t.Logf("Expected error: %v", err) }
}
...
func TestWriteMailStream_WithData(t *testing.T) {
	...
	if err := data.WriteMailStream(tmpFile, "file.eml"); err != nil { t.Logf("Expected error: %v", err) }
}
