...
func PrintQueryHelp(w io.Writer, parser string) {
	...
	case "basic":
		...
		_, _ = fmt.Fprintln(w, "\tfilter not c.job icontains .Software")
		_, _ = fmt.Fprintln(w, "Which filters all row's `job` column that does not contain `Software` ")
		_, _ = fmt.Fprintln(w, "Then you can filter the columns using:")
		_, _ = fmt.Fprintln(w, "\tinto table c.job f.year[c.startdate] f.month[c.startdate]")
		_, _ = fmt.Fprintln(w, "If you wanted a summary / count of the lines you can use the summary converter:")
		_, _ = fmt.Fprintln(w, "\tinto summary c.job calculate f.sum[c.pay] f.count")
		_, _ = fmt.Fprintln(w, "Which groups the row on job, then creates a sum of pay and a count of elements grouped")
		_, _ = fmt.Fprintln(w, "If you want to sort you can use:")
		_, _ = fmt.Fprintln(w, "\tsort f.year[c.startdate] f.month[c.startdate]")
		_, _ = fmt.Fprintln(w, "These can be used in any combination and repeated for the desired effect:")
		_, _ = fmt.Fprintln(w, "\tfilter c.startdate icontains .Software into summary c.job f.year[c.startdate] calculate f.count filter c.year-startdate eq 2022 sort c.job")
		_, _ = fmt.Fprintln(w, "And so forth")
		_, _ = fmt.Fprintln(w, "")
		_, _ = fmt.Fprintln(w, "Notes:")
		_, _ = fmt.Fprintln(w, "- Single word string literals begin with a `.`")
		_, _ = fmt.Fprintln(w, "- Columns are referred to by `c.`")
