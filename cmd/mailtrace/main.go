...
func PrintQueryHelp(w io.Writer, parser string) {
	...
	case "basic":
		...
		_, _ = fmt.Fprintln(w, "\tfilter not h.user-agent icontains .Kmail")
		_, _ = fmt.Fprintln(w, "Which filters all emails sent with a user agent that does not contain `kmail` ")
		_, _ = fmt.Fprintln(w, "Then you can convert the emails to tabular form using:")
		_, _ = fmt.Fprintln(w, "\tinto table h.user-agent h.subject f.year[h.date] f.month[h.date]")
		_, _ = fmt.Fprintln(w, "Which creates table of user agent, subject, and the year and month.")
		_, _ = fmt.Fprintln(w, "If you wanted a summary / count of the lines you can use the summary converter:")
		_, _ = fmt.Fprintln(w, "\tinto summary h.user-agent h.subject f.year[h.date] f.month[h.date] calculate f.sum[c.size] f.count")
		_, _ = fmt.Fprintln(w, "Which groups the mail based on user-agent, subject, year, month, then creates a sum and a count")
		_, _ = fmt.Fprintln(w, "If you want to sort you can use:")
		_, _ = fmt.Fprintln(w, "\tsort f.year[h.date] f.month[h.date]")
		_, _ = fmt.Fprintln(w, "These can be used in any combination and repeated for the desired effect:")
		_, _ = fmt.Fprintln(w, "\tfilter h.user-agent icontains .Kmail into summary h.user-agent f.year[h.date] calculate f.count filter c.year-date eq 2022 sort h.user-agent")
		_, _ = fmt.Fprintln(w, "And so forth")
		_, _ = fmt.Fprintln(w, "")
		_, _ = fmt.Fprintln(w, "Notes:")
		_, _ = fmt.Fprintln(w, "- Single word string literals begin with a `.`")
		_, _ = fmt.Fprintln(w, "- Headers are referred to with `h.` once in table form it becomes a column referred to by `c.`")
		_, _ = fmt.Fprintln(w, "- I haven't implemented any body parsing components yet")
		_, _ = fmt.Fprintln(w, "- Extension PRs are welcome and intended")
		_, _ = fmt.Fprintln(w, "- All functions are preceded by `f.`")
		_, _ = fmt.Fprintln(w, "- Once converted to table form, it can not be converted back to mailfile or mbox")
	}
