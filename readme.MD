...
```bash
toolname -input <file> -input-type <type> -parser basic -output-type <type> [QUERY]
```

*   `-input`: The source file (defaults to stdin `-`).
*   `-input-type <type>`: Specifies the input format. (Defaults to `list`, which prints available types and exits).
*   `-output-type <type>`: Specifies the output format. (Defaults to `list`, which prints available types and exits).
*   `-parser basic`: **Required.** Specifies the query parser to use.
*   `[QUERY]`: The sequence of operations to perform on the data.

## Exit Codes and Streams

PIMTrace tools follow a consistent contract for exit codes and standard streams:

### Exit Codes
*   **`0` (Success):** The query executed successfully. Explicit help (`-help`) or version (`-version`) requests also exit with `0`.
*   **`1` (Runtime Error):** An error occurred while reading input, parsing data, executing the query, or writing output.
*   **`2` (Usage Error):** Invalid flags, missing required arguments (like the query itself), or invalid query syntax.

### Streams
*   **`stdout` (Standard Output):** Reserved exclusively for the actual query results, or explicit help/version output. This ensures that the output is safe to pipe to other tools or redirect to files.
*   **`stderr` (Standard Error):** Used for all diagnostic messages, progress indicators, warnings, and error messages.
...
Conditions are constructed using **comparison operators** and combined using **boolean operators**:

| Operator | Description | Example |
| :--- | :--- | :--- |
| `eq` | Textual equality check. | `filter h.From eq .admin@example.com` |
...
| `gt` | Greater than (numeric or chronological date). | `filter c.Amount gt .50` |
| `gte` | Greater than or equal (numeric or chronological date). | `filter c.Amount gte .50` |
| `lt` | Less than (numeric or chronological date). | `filter p.DTSTART lt .2020-01-03` |
| `lte` | Less than or equal (numeric or chronological date). | `filter p.DTSTART lte .2020-01-03` |

#### Boolean Composition and Grouping

*   **`and`**: Logical AND. Binds more tightly than `or`.
*   **`or`**: Logical OR.
*   **`not`**: Unary logical NOT. Binds most tightly.
*   **Precedence**: `not` > `and` > `or`. In an expression like `A or B and C`, `B and C` is evaluated first.
*   **Parentheses / Grouping**: Use `(` and `)` to override default precedence: `( A or B ) and C`.
*   **Shell Quoting / Escaping**: In shells such as Bash or Zsh, unquoted parentheses trigger subshell syntax. Escape them with backslashes (e.g. `\(`, `\)`) or pass them quoted (e.g. `'('`, `')'`).

#### Typed Comparison & Coercion Rules

*   Relational operators (`gt`, `gte`, `lt`, `lte`) perform typed numeric (integer/decimal) or chronological date comparisons.
*   Legacy equality (`eq`) and substring operators (`contains`, `icontains`) remain textual (e.g., `"01"` and `"1"` are not equal under `eq`).
*   **Invalid Coercion Behavior**: Once numeric or date relational comparison is selected, operands must successfully coerce to the expected type. If an operand cannot be coerced (e.g. comparing invalid text like `apple` or an empty string against a number or date), evaluation produces an explicit error rather than silently falling back to lexical ordering or zero values.
...
```bash
csvtrace -input expenses.csv -input-type csv -parser basic -output-type table \
  filter c.Category icontains .Food \
  into table c.Date c.Amount
```

**Task:** Show me all large expenses (over $50).
```bash
csvtrace -input expenses.csv -input-type csv -parser basic -output-type table \
...
**Task:** Complex conditions using AND, OR, grouping and NOT. (Note shell escaping for parentheses).
```bash
csvtrace -input expenses.csv -input-type csv -parser basic -output-type table \
  filter \( c.Category eq .Food or c.Category eq .Travel \) and not c.Amount lt .10 \
...
```bash
csvtrace -input expenses.csv -input-type csv -parser basic -output-type plot.bar -output expenses_plot.png \
  into summary c.Category calculate f.sum[c.Amount]
```
...
```bash
mailtrace -input inbox.mbox -input-type mbox -parser basic -output-type table \
...
```bash
icaltrace -input calendar.ics -input-type ical -parser basic -output-type table \
...
Find events longer than 1 hour (3600 seconds):

```bash
icaltrace -input calendar.ics -input-type ical -parser basic -output-type table \
  into table p.SUMMARY f.duration \
  filter c.duration gt .3600
```
...
*   **List**: Prints available formats (this is the default behavior if `-input-type` or `-output-type` are omitted).
